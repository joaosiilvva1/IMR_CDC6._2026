...
É chamada de discreta porque o cálculo é feito em pequenos intervalos de tempo fixos, e não de forma contínua. Como a posição é recalculada passo a passo, pequenos erros aparecem a cada atualização e vão se somando ao longo do percurso.
No Exercício 2 isso ficou claro: depois de percorrer o quadrado, o robô não voltou exatamente ao ponto de partida. No nosso teste, o erro final foi de cerca de 10,6 px de distância e 3,9° de orientação em relação ao início.
Sobre a navegação GO-TO-GOAL: é uma técnica para fazer o robô chegar até um ponto determinado. Primeiro ele calcula a direção do alvo e compara com sua orientação atual; a partir dessa diferença, decide o quanto precisa girar.
Quanto maior o erro de direção, maior a correção aplicada — por isso chamamos de controlador proporcional. Um cuidado que tivemos na implementação foi normalizar esse erro de ângulo: sem isso, um erro de 340°, por exemplo, seria tratado ao pé da letra e faria o robô girar pelo caminho mais longo,
...
Em resumo, os exercícios ajudaram a entender como o robô representa sua posição, como as rodas controlam o movimento e como ele consegue navegar até um destino. Também vimos na prática que erros de cálculo se acumulam durante o percurso — algo 
